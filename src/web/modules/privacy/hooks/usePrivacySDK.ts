...
type UpdatedPoolAccount = PoolAccount & {
  ...
  scope: Hash
  chainId: number
  ragequit?: RagequitEvent & { timestamp?: bigint }
}
...
const usePrivacySDK = () => {
  ...
  console.log('DEBUG: data', pools, poolsByChain, chainDataByWhitelistedChains, chainData)
  ...
  useEffect(() => {
    ...
    const initializeSDK = async () => {
      ...
      try {
        ...
        console.log('DEBUG: sdk, circuits', sdk, circuits)

        const dataServiceConfig: ChainConfig[] = poolsByChain.map((pool: PoolInfo) => {
          return {
  ...
  }, [sdkState.isInitialized, sdkState.isLoading, poolsByChain, chainDataByWhitelistedChains])
  ...
  const getPoolAccountsFromAccount = async (chainId: number) => {
    ...
    const poolAccounts: UpdatedPoolAccount[] = []
    ...
    Array.from(paMap).forEach(([scope, poolAccountsArray]) => {
      poolAccountsArray.forEach(async (poolAccount, idx) => {
        ...
        const chainIdKey = chainData
          ? Object.keys(chainData).find((key) =>
        ...
        const updatedPoolAccount: UpdatedPoolAccount = {
          ...
          balance: lastCommitment?.value || 0n,
          lastCommitment,
        ...
        updatedPoolAccount.deposit = {
          ...updatedPoolAccount.deposit,
          timestamp: await getTimestampFromBlockNumber(
        ...
        if (updatedPoolAccount.children.length > 0) {
          updatedPoolAccount.children = await Promise.all(
            updatedPoolAccount.children.map(async (child) => ({
              ...child,
              timestamp: await getTimestampFromBlockNumber(child.blockNumber, publicClient)
            }))
          )
        }
    ...
    const poolAccountsByChainScope = poolAccounts.reduce((acc, curr) => {
      ...
      return acc
    }, {} as Record<string, UpdatedPoolAccount[]>)
